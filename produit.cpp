...
void Produit::rechercherProduits(const QString &motCle, QTableView *tableView) {
    ...
    // 2. Si la recherche est vide, afficher tous les produits
    if (motCle.isEmpty()) {
        afficherProduits(tableView);
        return;
    }

    // 3. Nouveau modèle et requête SQL optimisée
    QSqlQueryModel *model = new QSqlQueryModel();
    QString queryStr =
        "SELECT * FROM produits WHERE "
        "(LOWER(NOM) LIKE LOWER(:nom) OR "  // Insensible à la casse
        "LOWER(GAMME) LIKE LOWER(:gamme))";  // Retrait de CATEGORIE

    QSqlQuery query;
    query.prepare(queryStr);

    // 4. Ajout des wildcards (%) pour une recherche partielle + nettoyage du terme
    QString searchTerm = "%" + motCle.trimmed() + "%";
    ...
    // 6. Debug : Afficher la requête et le nombre de résultats
    qDebug() << "Requête exécutée:" << query.lastQuery();
    qDebug() << "Nombre de résultats:" << query.size();

    // 7. Assigner le modèle et forcer le rafraîchissement
    model->setQuery(std::move(query));  // C++11 : évite une copie inutile
    tableView->setModel(model);
    tableView->resizeColumnsToContents();  // Ajustement automatique des colonnes
}
